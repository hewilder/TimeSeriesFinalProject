...
library(forecast)
beerShort<-ts(subset.ts(beerTS, (time(beerTS)>=1979) & (time(beerTS)< 1985)), start=c(1979,1), frequency=12) 
seasonplot(beerShort,year.labels=TRUE,ylab="megaliters",main="Seasonal plot: quarterly beer production", col=rainbow(20), pch=19)
```
...
sumSqErrPop<-sum((log(beer_forecast)-pred)^2)
...
chosenMod<-ar12
modelString<-paste("Seasonal Means-", getModelString(chosenMod))
